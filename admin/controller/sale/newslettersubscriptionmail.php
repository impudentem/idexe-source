...
class ControllerSaleNewslettersubscriptionmail extends Controller {
	...
	public function index() {
		...
		$data['cancel'] = $this->url->link('sale/newslettersubscriptionmail', 'token=' . $this->session->data['token'], true);
...
/*
		...
		$this->response->setOutput($this->load->view('sale/newslettersubscriptionmail', $data));
	}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$json) {
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('sale/newslettersubscription/send', 'token=' . $this->session->data['token'] . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
