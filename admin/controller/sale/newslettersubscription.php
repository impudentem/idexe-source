...
class ControllerSaleNewslettersubscription extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('sale/newslettersubscription', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/newslettersubscription', 'token=' . $this->session->data['token'] . $url, true)
		);
		
		$data['add'] = $this->url->link('sale/newslettersubscription/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('sale/newslettersubscription/delete', 'token=' . $this->session->data['token'] . $url, true);
		//1
...
//1
		...
		foreach ($results as $result) {
			$data['newslettersubscriptions'][] = array(
				...
				'edit'       => $this->url->link('sale/newslettersubscription/edit', 'token=' . $this->session->data['token'] . '&subscription_id=' . $result['subscription_id'] . $url, true)
		...
		$data['sort_status'] = $this->url->link('sale/newslettersubscription', 'token=' . $this->session->data['token'] . '&sort=r.status' . $url, true);
		$data['sort_date_added'] = $this->url->link('sale/newslettersubscription', 'token=' . $this->session->data['token'] . '&sort=r.create_time' . $url, true);
		$data['sort_email'] = $this->url->link('sale/newslettersubscription', 'token=' . $this->session->data['token'] . '&sort=r.email' . $url, true);
		...
		$pagination->url = $this->url->link('sale/newslettersubscription', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('sale/newslettersubscription', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['subscription_id'])) {
			$data['action'] = $this->url->link('sale/newslettersubscription/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/newslettersubscription/edit', 'token=' . $this->session->data['token'] . '&subscription_id=' . $this->request->get['subscription_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/newslettersubscription', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('sale/newslettersubscription_form', $data));
	}
