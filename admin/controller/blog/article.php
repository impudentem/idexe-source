...
class ControllerBlogArticle extends Controller {
    ...
    public function switchArticleStatus() {
        $this->load->language('blog/article');
        ...
        if (isset($this->request->get['article_id']) && isset($this->request->get['to_status'])) {

            if ($this->request->get['to_status']){
                $this->model_blog_article->setArticleStatus($this->request->get['article_id'], 1);
            } else {
                $this->model_blog_article->setArticleStatus($this->request->get['article_id'], 0);
            }
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['articles'][] = array(
                ...
                'comments_unpublished' => $result['comments_unpublished'],
				'status'               => $result['status'],
				'status_link'          => ($result['status']) ? $this->url->link('blog/article/switchArticleStatus', 'token=' . $this->session->data['token'] . '&article_id=' . $result['article_id']. '&to_status=0'  . $url, true) : $this->url->link('blog/article/switchArticleStatus', 'token=' . $this->session->data['token'] . '&article_id=' . $result['article_id']. '&to_status=1'  . $url, true),
                'comments_total_link'       => $this->url->link('blog/comment', 'token=' . $this->session->data['token'] . '&filter_article_id=' . $result['article_id'] . $url, true),
                'comments_unpublished_link' => $this->url->link('blog/comment', 'token=' . $this->session->data['token'] . '&filter_article_id=' . $result['article_id'] . '&filter_status=0'  . $url, true),
        ...
        $data['help_comments_total'] = $this->language->get('help_comments_total');
        $data['help_comments_unpublished'] = $this->language->get('help_comments_unpublished');
        ...
        $data['sort_date'] = $this->url->link('blog/article', 'token=' . $this->session->data['token'] . '&sort=a.date_modified' . $url, true);
        $data['sort_views'] = $this->url->link('blog/article', 'token=' . $this->session->data['token'] . '&sort=a.viewed' . $url, true);
        $data['sort_comments'] = $this->url->link('blog/article', 'token=' . $this->session->data['token'] . '&sort=comments_total' . $url, true);
	...
	protected function getForm() {
        ...
        $data['entry_status'] = $this->language->get('entry_status');
        $data['entry_stick'] = $this->language->get('entry_stick');
        ...
        if (isset($this->request->post['stick'])) {
            $data['stick'] = $this->request->post['status'];
        } elseif (!empty($article_info)) {
            $data['stick'] = $article_info['stick'];
        } else {
            $data['stick'] = true;
        }
		...
		$data['article_blog_categories'] = array();
		...
		foreach ($categories as $blog_category_id) {
			...
			if ($category_info) {
				$data['article_blog_categories'][] = array(
					...
					'name' => ($category_info['blog_path']) ? $category_info['blog_path'] . ' &gt; ' . $category_info['name'] : $category_info['name']
				);
			}
		}
        ...
        //Main article image
        $this->load->model('tool/image');
		if (isset($this->request->post['image_main'])) {
			$data['image_main'] = $this->request->post['image_main'];
		} elseif (!empty($article_info)) {
			$data['image_main'] = $article_info['image_main'];
		} else {
			$data['image_main'] = '';
		}

		if (isset($this->request->post['image_main']) && is_file(DIR_IMAGE . $this->request->post['image_main'])) {
			$data['thumb'] = $this->model_tool_image->resize($this->request->post['image_main'], 100, 100);
		} elseif (!empty($article_info) && is_file(DIR_IMAGE . $article_info['image_main'])) {
			$data['thumb'] = $this->model_tool_image->resize($article_info['image_main'], 100, 100);
		} else {
