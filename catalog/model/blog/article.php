...
class ModelBlogArticle extends Model {
    ...
    public function getArticle($article_id) {
        $query = $this->db->query("SELECT DISTINCT a.article_id, a.sort_order, a.stick, a.status, a.image_main, a.date_modified, a.viewed, ad.language_id, ad.name, ad.description, ad.intro_text, ad.meta_title, ad.meta_description, ad.meta_keyword, ad.author, ad.tag, (SELECT COUNT(*) FROM " . DB_PREFIX . "blog_comment c WHERE c.article_id = a.article_id AND c.status=1) AS comments_published FROM " . DB_PREFIX . "blog_article a LEFT JOIN " . DB_PREFIX . "blog_article_description ad ON (a.article_id = ad.article_id) WHERE a.article_id = '" . (int)$article_id . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND a.status = '1' ");

		if ($query->num_rows) {
			return array(
				'article_id'       => $query->row['article_id'],
				'name'             => $query->row['name'],
				'image_main'       => $query->row['image_main'],
    ...
    public function getArticles($data = array()) {
        ...
        if (!empty($data['filter_blog_category_id'])) {
            if (!empty($data['filter_sub_category'])) {
                $sql .= " FROM " . DB_PREFIX . "blog_category_path ca LEFT JOIN " . DB_PREFIX . "blog_article_to_blog_category a2c ON (ca.blog_category_id = a2c.blog_category_id)";
            } else {
                $sql .= " FROM " . DB_PREFIX . "blog_article_to_blog_category a2c";
            }
    ...
    public function getCategoriesWithName($article_id) {
        $query = $this->db->query("SELECT atc.blog_category_id, cd.name FROM " . DB_PREFIX . "blog_category_description cd LEFT JOIN " . DB_PREFIX . "blog_article_to_blog_category atc ON cd.blog_category_id = atc.blog_category_id WHERE atc.article_id = '" . (int)$article_id . "'");
	...
	public function getTotalArticles($data = array()) {
		...
		if (!empty($data['filter_blog_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				$sql .= " FROM " . DB_PREFIX . "blog_category_path cp LEFT JOIN " . DB_PREFIX . "blog_article_to_blog_category a2c ON (cp.blog_category_id = a2c.blog_category_id)";
			} else {
				$sql .= " FROM " . DB_PREFIX . "blog_article_to_blog_category a2c";
			}
			$sql .= " LEFT JOIN " . DB_PREFIX . "blog_article a ON (a2c.article_id = a.article_id)";
