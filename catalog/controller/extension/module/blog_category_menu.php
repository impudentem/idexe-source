	...
	public function index() {
		...
		foreach ($blog_categories as $blog_category) {
			...
			if ($blog_category['blog_category_id'] == $data['blog_category_id']) {
				...
				foreach($children as $child) {
					...
					$children_data[] = array(
                        ...
                        'name' => $child['name'] . ($this->config->get('oc_blog_category_menu_article_count') ? ' (' . $this->model_blog_article->getTotalArticles($filter_data) . ')' : ''),
			...
			$data['blog_categories'][] = array(
                ...
                //TODO config articles count
                'name'        => $blog_category['name'] . ($this->config->get('oc_blog_category_menu_article_count') ? ' (' . $this->model_blog_article->getTotalArticles($filter_data) . ')' : ''),
				'children'    => $children_data,
				'href'        => $this->url->link('blog/blog_category', 'blog_path=' . $blog_category['blog_category_id'])
			);
		}
		
        return $this->load->view('module/blog_category_menu', $data);
	}
}