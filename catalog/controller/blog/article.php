...
class ControllerBlogArticle extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->config->get('oc_blog_home_page_name'),
		...
		if ($article_info && ($this->config->get('oc_blog_global_status'))) {
            ...
            if ($this->config->get('oc_blog_home_page_meta_title')) {
                $this->document->setTitle($this->config->get('oc_blog_home_page_meta_title') . ' - ' . $article_info['meta_title']);
            } else {
                $this->document->setTitle($article_info['meta_title']);
            }

            if ($this->config->get('oc_blog_home_page_meta_description')){
                $this->document->setDescription($this->config->get('oc_blog_home_page_meta_description') . ' ' .$article_info['meta_description']);
            } else {
                $this->document->setDescription($article_info['meta_description']);
            }

            if ($this->config->get('oc_blog_home_page_meta_keyword')){
                $this->document->setKeywords($this->config->get('oc_blog_home_page_meta_keyword') . ' ' .$article_info['meta_keyword']);
            } else {
                $this->document->setKeywords($article_info['meta_keyword']);
            }

            if ($this->config->get('oc_blog_comment_captcha_status')) {
                $this->document->addScript('https://www.google.com/recaptcha/api.js');
                $data['site_key'] = $this->config->get('oc_blog_comment_captcha_public');
            } else {
                $data['site_key'] = '';
            }

			$data['show'] = array(
				'date' => $this->config->get('oc_blog_article_show_date'),
				'author' => $this->config->get('oc_blog_article_show_author'),
				'view' => $this->config->get('oc_blog_article_show_viewed'),
				'comment' => $this->config->get('oc_blog_article_show_number_of_comments'),
				'category' => $this->config->get('oc_blog_article_show_category'),
				'tag'=> $this->config->get('oc_blog_article_show_tag')
			);
            ...
            $data['comment_status']=$this->config->get('oc_blog_comment_config_status');
			if ($this->config->get('oc_blog_comment_config_guest') || $this->customer->isLogged()) {
    ...
    public function write() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ($this->config->get('oc_blog_comment_captcha_status') && empty($json['error'])) {
                if (isset($this->request->post['g-recaptcha-response'])) {
                    $recaptcha = file_get_contents('https://www.google.com/recaptcha/api/siteverify?secret=' . urlencode($this->config->get('oc_blog_comment_captcha_secret')) . '&response=' . $this->request->post['g-recaptcha-response'] . '&remoteip=' . $this->request->server['REMOTE_ADDR']);
            ...
            if (!isset($json['error'])) {
				...
				if ($this->config->get('oc_blog_comment_config_approve')){
