...
class ControllerBlogBlogCategory extends Controller {
	public function index() {
		...
		if ($blog_category_info && ($this->config->get('oc_blog_global_status'))) {
			if ($this->config->get('oc_blog_home_page_meta_title')) {
                $this->document->setTitle($this->config->get('oc_blog_home_page_meta_title') . ' - ' . $blog_category_info['meta_title']);
            } else {
                $this->document->setTitle($blog_category_info['meta_title']);
            }

            if ($this->config->get('oc_blog_home_page_meta_description')){
                $this->document->setDescription($this->config->get('oc_blog_home_page_meta_description') . ' ' .$blog_category_info['meta_description']);
            } else {
                $this->document->setDescription($blog_category_info['meta_description']);
            }

            if ($this->config->get('oc_blog_home_page_meta_keyword')){
                $this->document->setKeywords($this->config->get('oc_blog_home_page_meta_keyword') . ' ' .$blog_category_info['meta_keyword']);
            } else {
			...
			foreach ($results as $result) {
                ...
                if ($result['image_main']) {
                    $image_main = $this->model_tool_image->cropsize($result['image_main'], 392, 364);
			...
			$data['show'] = array(
				'date' => $this->config->get('oc_blog_category_show_date'),
				'author' => $this->config->get('oc_blog_category_show_author'),
				'view' => $this->config->get('oc_blog_category_show_viewed'),
				'comment' => $this->config->get('oc_blog_category_show_number_of_comments'),
				'category' => $this->config->get('oc_blog_category_show_category'),
				'tag'=> $this->config->get('oc_blog_category_show_tag')
			);
			...
			$limits = array_unique(array($this->config->get('oc_blog_global_article_limit'), 50, 75, 100));
