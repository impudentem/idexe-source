...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (substr(str_replace('\\', '/', realpath($directory . '/' . $filter_name)), 0, strlen(DIR_IMAGE . 'catalog')) == DIR_IMAGE . 'catalog') {
			...
			// Get files
			$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,pdf,mp4,webm,ogv,zip,xls,xlsx,doc,docx,ppt,pptx,odt,ods,odp,JPG,JPEG,PNG,GIF,PDF,MP4,WEBM,OGV,ZIP,XLS,DOC,PPT,ODT,ODS,ODP}', GLOB_BRACE);
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$extraextensions = array('xls', 'xlsx', 'doc', 'docx', 'ppt', 'pptx', 'odt', 'ods', 'odp', 'zip', 'pdf', 'mp4', 'webm', 'ogv');
		...
		// Return the CKEditor callback function
			...
			foreach ($files as $file) {
				if (is_file($file['tmp_name'])) {
					...
					$allowed = array(
						...
						'gif',
						'png'
,'xls', 'xlsx', 'doc', 'docx', 'ppt', 'pptx', 'odt', 'ods', 'odp', 'zip', 'pdf', 'mp4', 'webm', 'ogv'
					);
