...
class ControllerExtensionPaymentMastercardPGS extends Controller {
	...
	public function index() {
		...
		if (version_compare(VERSION, '2.3.0.0', '<')) {
			$data['error_event_missing'] = $this->language->get('error_event_missing_old');
		} else {
		...
		if ($this->customer->isLogged()) {
			$data['cards'] = $this->imodule_model->getCards($this->customer->getId());
		} else {
			$data['cards'] = array();
		}
		...
		if (
			...
			$this->imodule_model->initCheckoutSession($this->session->data['order_id'], $this->session->data['currency'])
		) {
		...
		$data['imodule_route'] = $this->imodule_route;

		if (version_compare(VERSION, '2.2.0.0', '>=')) {
			$tpl_path = $this->imodule_route;
		} else {
			$tpl_path = $this->config->get('config_template') . '/template/' . $this->imodule_route . '.tpl';
		}
	...
	public function js() {
		...
		$data['complete'] = $this->url->link($this->imodule_route . '/success', '', true);
		...
		$data['checkout_script'] = $this->imodule_model->getGateway() . '/checkout/version/' . $this->imodule_model->getApiVersion() . '/checkout.js';

		$this->response->addHeader("Content-Type:application/javascript");

		if (version_compare(VERSION, '2.2.0.0', '>=')) {
			$tpl_path = $this->imodule_route . '_js';
		} else {
			$tpl_path = $this->config->get('config_template') . '/template/' . $this->imodule_route . '_js.tpl';
		}
	...
	public function checkout() {
		...
		$json = array(
			'redirect' => $this->url->link($this->imodule_route . '/success', '', true)
		);
		...
		if ($this->request->server['REQUEST_METHOD'] != 'POST' || empty($this->request->post['token']) || @base64_decode($this->request->post['token']) === FALSE) {
			...
			$response = $this->imodule_model->api('order/' . $this->session->data['order_id'] . '/transaction/1', $configuration, 'PUT');
	...
	public function success() {
		...
		if (
			...
			$session_data = $this->imodule_model->api('session/' . $this->session->data['mastercard_pgs']['session']['id']);
			...
			if (!empty($session_data['billing']['address']) && !empty($session_data['interaction']['displayControl']['billingAddress']) && in_array($session_data['interaction']['displayControl']['billingAddress'], array('MANDATORY', 'OPTIONAL'))) {
				$this->imodule_model->editOrderPaymentDetails($this->session->data['order_id'], $session_data['billing']['address']);
			}

			if ($this->config->get('mastercard_pgs_tokenize') && empty($session_data['sourceOfFunds']['token'])) {
				$this->imodule_model->saveToken($this->session->data['order_id'], token(40), $this->session->data['mastercard_pgs']['session']['id'], $session_data['sourceOfFunds']);
			}
		}

		$this->imodule_model->clearCheckoutSession();
	...
	public function callback() {
		...
		if ($this->config->get('mastercard_pgs_debug_log')) {
			$log_data = $this->imodule_model->getNotificationLogData();
		...
		$transaction_info = $this->imodule_model->parseWebhookNotification();

		if ($transaction_info) {
			$transaction_info['partnerSolutionId'] = $this->imodule_model->getPartnerSolutionId();
			...
			$is_transaction_logged = $this->imodule_model->isTransactionLogged($transaction_info['transaction']['id'], $transaction_info['order']['reference']);

			if (!$is_transaction_logged) {
				$this->imodule_model->saveTransaction($transaction_info);

				// last check if everything went okay
				$is_transaction_logged = $this->imodule_model->isTransactionLogged($transaction_info['transaction']['id'], $transaction_info['order']['reference']);
			} else {
				$this->imodule_model->updateTransaction($transaction_info);
		...
		if (!$is_transaction_logged) {
			...
			switch ($risk_gateway_code) {
				case 'REJECTED' : {
					$this->imodule_model->addOrderHistory(array(
	...
	protected function validate_callback() {
		...
		if (!$this->config->get('mastercard_pgs_notification_secret') || $this->config->get('mastercard_pgs_notification_secret') != $this->imodule_model->getHeaderVar('HTTP_X_NOTIFICATION_SECRET')) {
	...
	protected function configuration($hosted = true) {
		...
		$c['partnerSolutionId'] = $this->imodule_model->getPartnerSolutionId();
